# Ashton & Price Implementation Plan (client-facing shape)

> **Internal note (not for the firm).** This is the standing client-facing
> shape of `IMPLEMENTATION-PLAN.md`: the same plan in the firm's language.
> Captain reviews before any of it is sent or spoken from. It carries no
...
We do not run this rollout on a calendar; we run it on proof. Each stage
starts when the one before it has shown it works, and the pace is set by
the evidence and by your availability for the items above, not by dates we
would have to guess at today.
