...
3. **Defense in depth.** A separate code-level filter (`operator/safety-substrate/citation_filter.py`) enforces the prohibition at the output layer. This document enforces it at the prompt layer.
