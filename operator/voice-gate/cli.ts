 ...
 *
 * Usage:
 *   npx tsx operator/voice-gate/cli.ts \
...
function usageAndExit(reason: string): never {
  ...
  console.error(
    'Usage: npx tsx operator/voice-gate/cli.ts --customer-slug <slug> [--cohort <cohort>] --panel-id <id> [--cycle-count N] [--mode synthetic|live] [--identifications path] [--allow-undersized]'
  )
  process.exit(2)
}
