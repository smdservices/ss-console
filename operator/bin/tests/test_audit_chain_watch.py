...
# ---------------------------------------------------------------------------
# The alarm drill (ss#2500, --rehearse-mismatch)
#
# The criterion these cover is "alert fires on a synthetic head mismatch". The
# detector was already well covered; what was never exercised is the path from a
# finding to the sink, and the three things a drill must not do: overwrite a real
# alert, write to the object-locked archive, or leave its own fake alarm up.
# ---------------------------------------------------------------------------
...
class _FakeConsole:
    ...
    def __init__(self, *, pin=None, entity="ent-1", fail_clear=False, fail_write=False):
        self._pin = pin
        self._entity = entity
    ...
    def entity_id(self, slug):
        return self._entity

    def write_alert(self, *, entity_id, slug, summary, details, driver_prefix):
        if self._fail_write:
            raise RuntimeError("d1 refused the insert")
        self.writes.append(
            {
                "slug": slug,
                "summary": summary,
...
def _seam(monkeypatch, rows):
    class _Client:
        def read_all(self, _table):
            return rows

    monkeypatch.setattr(watch, "seam_client_from_env", lambda slug: _Client())


def test_the_drill_fires_the_alarm_and_then_clears_it(monkeypatch):
    rows = _chain(20)
    console = _FakeConsole(pin=_pin(rows, -1))
    _seam(monkeypatch, rows)

    code, lines = watch.rehearse_mismatch("smd-staging", console)

    assert code == watch.EXIT_REHEARSAL_OK
    assert len(console.writes) == 1, "the alarm did not fire"
    assert console.cleared == ["smd-staging"], "the drill left its own alarm standing"
    assert any("the alarm fired" in ln for ln in lines)


def test_the_drill_writes_under_the_rehearsal_driver_not_the_real_one(monkeypatch):
    ...
    watch.rehearse_mismatch("smd-staging", console)

    prefix = console.writes[0]["driver_prefix"]
    assert prefix == watch.REHEARSAL_DRIVER_PREFIX
    assert prefix != watch.ALERT_DRIVER_PREFIX


def test_the_drill_row_says_it_is_synthetic_in_the_summary_and_the_details(monkeypatch):
    rows = _chain(12)
    console = _FakeConsole(pin=_pin(rows, -1))
    _seam(monkeypatch, rows)
...
def test_the_drill_never_archives(monkeypatch):
    """The audit/ prefix is object-locked for seven years; a drill has no
    business writing there, and a key it burned could never be rewritten."""
    rows = _chain(12)
    console = _FakeConsole(pin=_pin(rows, -1))
    _seam(monkeypatch, rows)

    def _boom(*a, **k):
        raise AssertionError("the drill must not touch the archive")

    monkeypatch.setattr(watch, "archive_export", _boom)
    assert watch.rehearse_mismatch("smd-staging", console)[0] == watch.EXIT_REHEARSAL_OK


def test_the_drill_leaves_the_real_pin_alone(monkeypatch):
    ...
    rows = _chain(12)
    real = _pin(rows, -1)
    console = _FakeConsole(pin=real)
    _seam(monkeypatch, rows)

    watch.rehearse_mismatch("smd-staging", console)

    assert real["audit_head"] == rows[-1]["row_hash"]
    assert console.writes[0]["details"]["pinned_head"] == watch.REHEARSAL_HEAD


def test_a_drill_that_produces_no_finding_fails(monkeypatch):
    ...
    the drill must go red rather than quietly succeed at nothing."""
    rows = _chain(12)
    console = _FakeConsole(pin=_pin(rows, -1))
    _seam(monkeypatch, rows)

    clean = watch.SeatOutcome("smd-staging", watch.CLEAN, "smd-staging: fine", {"head": "x"})
    monkeypatch.setattr(watch, "evaluate_export", lambda *a, **k: clean)

    code, lines = watch.rehearse_mismatch("smd-staging", console)
    assert code == watch.EXIT_REHEARSAL_FAILED
    assert console.writes == [], "a non-finding must not write an alert row"
    assert any("did not produce a finding" in ln for ln in lines)


def test_the_rehearsal_head_is_shaped_like_a_real_one(monkeypatch):
    ...
    assert len(watch.REHEARSAL_HEAD) == 64
    assert all(c in "0123456789abcdef" for c in watch.REHEARSAL_HEAD)
    assert watch.REHEARSAL_HEAD not in {r["row_hash"] for r in rows}

    watch.rehearse_mismatch("smd-staging", console)
    assert console.writes[0]["details"]["pin_verdict"] != "pin_malformed"


def test_a_failed_clear_is_reported_as_a_failure_with_the_row_to_delete(monkeypatch):
    """If the row cannot be cleared, the run must go red and say exactly what is
    still on the dashboard. Silence here leaves a fake integrity alarm up."""
    rows = _chain(12)
    console = _FakeConsole(pin=_pin(rows, -1), fail_clear=True)
    _seam(monkeypatch, rows)

    code, lines = watch.rehearse_mismatch("smd-staging", console)
    assert code == watch.EXIT_REHEARSAL_FAILED
    assert any(watch.REHEARSAL_DRIVER_PREFIX in ln for ln in lines)


def test_a_seam_that_is_not_configured_fails_rather_than_passing(monkeypatch):
    console = _FakeConsole()
    monkeypatch.setattr(watch, "seam_client_from_env", lambda slug: None)
    code, _lines = watch.rehearse_mismatch("smd-staging", console)
    assert code == watch.EXIT_REHEARSAL_FAILED
    assert console.writes == []


def test_the_drill_exit_codes_cannot_be_confused_with_a_real_verdict():
    """3 and 4 sit outside the clean/finding/hold vocabulary on purpose."""
    assert watch.EXIT_REHEARSAL_OK not in {
        watch.EXIT_CLEAN,
        watch.EXIT_FINDING,
        watch.EXIT_HOLD,
    }
    assert watch.EXIT_REHEARSAL_FAILED not in {
        watch.EXIT_CLEAN,
        watch.EXIT_FINDING,
        watch.EXIT_HOLD,
    }
...
def test_clear_rehearsal_alerts_can_only_delete_rehearsal_rows():
    ...
    console = watch.ConsoleD1(db="test-db", runner=runner)
    console.clear_rehearsal_alerts(slug="seat%' OR '1'='1")
    ...
    assert driver.startswith(watch.REHEARSAL_DRIVER_PREFIX)
