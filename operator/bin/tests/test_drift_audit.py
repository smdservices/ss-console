...
# --------------------------------------------------------------------------- #
# plan_actions — the D-act drafting decision logic
# --------------------------------------------------------------------------- #


def _f(cls, sev, corrective, slug="smd", key="k"):
    return da.Finding(slug, cls, sev, key, "detail", corrective)


def test_critical_acts_on_first_detection_and_gates() -> None:
    crit = _f("strip_violation", "critical", "live_flag", key="R2_X")
    actions, next_seen, waiting = da.plan_actions([crit], prev_seen=set(), degraded_slugs=set())
    assert len(actions) == 1
    assert actions[0].kind == "issue" and actions[0].gate is True
    assert actions[0].reason == "critical-immediate"
    assert da.finding_key(crit) in next_seen
    assert waiting == []


def test_noncritical_waits_for_second_run() -> None:
    warn = _f("cron_not_registered", "warn", "live_flag", key="inbox-triage")
    # First run: recorded, not acted.
    actions, next_seen, waiting = da.plan_actions([warn], prev_seen=set(), degraded_slugs=set())
    assert actions == []
    assert waiting == [warn]
    assert da.finding_key(warn) in next_seen
    # Second run (same finding in prev_seen): now acted.
    actions2, _, _ = da.plan_actions([warn], prev_seen=next_seen, degraded_slugs=set())
    assert len(actions2) == 1
    assert actions2[0].kind == "issue" and actions2[0].gate is False
    assert actions2[0].reason == "confirmed-2run"


def test_overlay_ref_pin_mismatch_drafts_pr_on_confirm() -> None:
    pin = _f("overlay_ref_pin_mismatch", "warn", "repo_patch", slug="*", key="pin")
    seen = {da.finding_key(pin)}
    actions, _, _ = da.plan_actions([pin], prev_seen=seen, degraded_slugs=set())
    assert len(actions) == 1 and actions[0].kind == "pr"


def test_block_authored_but_inert_is_issue_not_pr() -> None:
    # repo_patch but NOT mechanically patchable → issue, never an autogenerated patch.
    inert = _f("block_authored_but_inert", "info", "repo_patch", key="telegram")
    seen = {da.finding_key(inert)}
    actions, _, _ = da.plan_actions([inert], prev_seen=seen, degraded_slugs=set())
    assert len(actions) == 1 and actions[0].kind == "issue"


def test_resolved_finding_drops_from_state_when_read_clean() -> None:
    warn = _f("cron_not_registered", "warn", "live_flag", key="inbox-triage")
    seen = {da.finding_key(warn)}
    # Finding gone this run, slug read was clean → resolved, dropped.
    _, next_seen, _ = da.plan_actions([], prev_seen=seen, degraded_slugs=set())
    assert next_seen == set()


# --------------------------------------------------------------------------- #
# drift-audit-act.py helpers (deterministic, gh-free naming)
# --------------------------------------------------------------------------- #


def _load_act_module():
    import importlib.util

    path = _BIN / "drift-audit-act.py"
    spec = importlib.util.spec_from_file_location("drift_audit_act", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


def test_branch_name_sanitizes_repo_slug_and_is_deterministic() -> None:
    act = _load_act_module()
    repo_finding = _f("overlay_ref_pin_mismatch", "warn", "repo_patch", slug="*", key="pin")
    branch = act.branch_name(repo_finding)
    assert "*" not in branch  # not a legal git ref char
    assert branch.startswith("drift-audit/overlay_ref_pin_mismatch-")
    assert act.branch_name(repo_finding) == branch  # deterministic across calls


def test_issue_title_is_deterministic_and_keyed() -> None:
    act = _load_act_module()
    f = _f("cron_not_registered", "warn", "live_flag", slug="smd", key="inbox-triage")
    assert act.issue_title(f) == "[drift] cron_not_registered · smd:inbox-triage"


def test_degraded_read_carries_finding_forward_not_resolved() -> None:
    warn = _f("cron_not_registered", "warn", "live_flag", slug="smd", key="inbox-triage")
    seen = {da.finding_key(warn)}
    # Finding absent this run BUT smd had a degraded read → carried, NOT resolved,
    # so a flapping read can't bury it.
    actions, next_seen, waiting = da.plan_actions([], prev_seen=seen, degraded_slugs={"smd"})
    assert actions == []
    assert da.finding_key(warn) in next_seen
