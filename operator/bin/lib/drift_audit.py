...
# Finding classes whose corrective is a MECHANICAL edit between two repo
# artifacts — safe for the audit to draft as a one-line code PR. Everything else
# (including other repo_patch classes that need human judgement, and every
# live_flag class) becomes an issue, never an autogenerated patch.
PR_DRAFTABLE_CLASSES = frozenset({"overlay_ref_pin_mismatch"})


@dataclass(frozen=True)
class Action:
    """One thing D-act should do for a finding: open an issue, draft a PR, or
    (for criticals) also gate the run red. ``reason`` explains the trigger."""

    finding: Finding
    kind: str  # "pr" | "issue"
    gate: bool  # True => the run exits non-zero (critical, first detection)
    reason: str  # "critical-immediate" | "confirmed-2run"


def finding_key(f: Finding) -> str:
    """Stable identity for cross-run dedupe + deterministic branch/issue names."""
    return f"{f.cls}:{f.slug}:{f.key}"
...
def plan_actions(
    ...
    *,
    prev_seen: set[str],
    degraded_slugs: set[str],
) -> tuple[list[Action], set[str], list[Finding]]:
    """Decide what D-act does this run (pure — no GitHub, no network).

    Returns ``(actions, next_seen, waiting)``:
      * **actions** — issues/PRs to create now. Criticals act on FIRST detection
        (``gate=True``). Non-criticals act only once the SAME finding has been
        seen on two consecutive runs (it was in ``prev_seen``).
      * **next_seen** — the seen-set to persist for the next run.
      * **waiting** — findings seen for the first time (recorded, not yet acted)
        plus prior seen-once findings carried forward because their read was
        degraded this run (a flapping/degraded read must never bury a real drift,
        so it does NOT reset the counter).
    """
    actions: list[Action] = []
    ...
    for f in findings:
        ...
        if f.severity == "critical":
            actions.append(Action(f, "issue", True, "critical-immediate"))
            next_seen.add(key)
            continue
        if key in prev_seen:
            kind = "pr" if f.cls in PR_DRAFTABLE_CLASSES else "issue"
            actions.append(Action(f, kind, False, "confirmed-2run"))
            next_seen.add(key)
        else:
            waiting.append(f)
            next_seen.add(key)

    # Carry-forward: a prior seen-once key absent this run is "resolved" ONLY if
    # the read was clean. If the slug had ANY degraded read this run, we could not
    # confirm — keep it pending so a flapping read can't silently drop a real drift.
    for key in prev_seen:
        if key in current_keys:
            continue
        slug = key.split(":", 2)[1] if key.count(":") >= 2 else ""
        if slug in degraded_slugs:
            next_seen.add(key)  # carried, still unconfirmed

    return actions, next_seen, waiting
