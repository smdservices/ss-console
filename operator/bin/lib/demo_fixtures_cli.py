...
Invoked from ``operator/bin/load-demo-fixtures.sh``. The shell script
...
# operator/ on sys.path so the canonical `from bin.lib.demo_fixtures
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "operator/customers/. The loader refuses to run on a slug "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Override the fixtures root (defaults to operator/fixtures/ "
            "computed from this script's location). Used by tests."
        ),
    )
    return parser
...
def _resolve_aie_root(explicit: Path | None) -> Path:
    ...
    # operator/bin/lib/demo_fixtures_cli.py -> operator/
