...
* **Template-slug reservation.** Any slug whose first segment starts
  ...
  guarantee documented in ``operator/customers/_template/README.md``.
...
1. ``operator/customers/{slug}/customer.yaml`` exists and parses.
2. Voice samples directory has >=10 entries (counted from
   ``voice_samples_path``; defaults to ``operator/customers/{slug}/voice/``).
...
5. At least one synthetic matter fixture is seeded under
   ``operator/customers/{slug}/fixtures/`` OR the customer.yaml
   demo-angle field points at a checked-in fixture under
   ``operator/skills/{skill}/fixtures/`` /
   ``operator/fixtures/law-firm/``.
...
def is_template_slug(slug: str) -> bool:
    ...
    ``operator/customers/_template/README.md``.
    """
...
class MemoryStateReader(Protocol):
    ...
    ``docs/specs/operator/memory-ingestion.md``). Returns ``None``
...
class FilesystemMemoryReader:
    """Reads ``operator/customers/{slug}/.demo-prep-state.json``.
...
# ``operator/connectors/filevine/bin/smoke-test-filevine.py`` with
...
def _has_synthetic_matter_fixture(customer_dir: Path, customer_yaml: dict, fixture_search_paths: Iterable[Path]) -> tuple[bool, dict]:
    ...
    1. ``operator/customers/{slug}/fixtures/`` contains at least one
    ...
    2. The customer.yaml carries a ``demo.matter_fixture`` path that
       ...
       ``operator/skills/{skill}/fixtures/`` or
       ``operator/fixtures/law-firm/``. The path must resolve to an
...
class DemoPrepRunner:
    ...
    def run(self) -> list[CheckResult]:
        ...
        if not customer_dir.is_dir():
            raise DemoPrepPreflightError(
                ...
                "copy operator/customers/_template/ to populate it"
            )
    ...
    def _check_customer_yaml(self, path: Path, parsed: dict) -> CheckResult:
        ...
        # validation lives in src/lib/operator/customer-yaml/ (the
        ...
        # Memory isolation invariants per docs/specs/operator/r2-vectorize-naming.md.
    ...
    def _check_voice_samples(self, customer_dir: Path, customer_yaml: dict) -> CheckResult:
        # Voice samples live by default in operator/customers/{slug}/voice/
    ...
    def _check_memory_ingestion(self, customer_yaml: dict) -> CheckResult:
        # Per docs/specs/operator/memory-ingestion.md, ingestion
    ...
    def _check_voice_ingestion(self, customer_yaml: dict) -> CheckResult:
        # Per docs/specs/operator/voice-ingestion.md, ingestion
