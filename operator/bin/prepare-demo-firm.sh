...
#
# Usage:
#   operator/bin/prepare-demo-firm.sh --firm-slug <slug>
#   operator/bin/prepare-demo-firm.sh --firm-slug <slug> --min-voice-samples 10
#
# Exit codes:
...
#
#   1. Captain authors operator/customers/{slug}/customer.yaml and
#      dossier.md by copying operator/customers/_template/.
#   2. Captain runs operator/bin/provision-customer.sh {slug} which
...
# Run from inside operator/ so `bin.lib.demo_prep_cli` resolves as a
