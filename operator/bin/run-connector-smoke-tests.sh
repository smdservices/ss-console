...
#
# Invokes operator/adapter/run_prod_smoke_test.py with the customer's
...
CUSTOMER_YAML="${REPO_ROOT}/operator/customers/${SLUG}/customer.yaml"
...
# script handles its own sys.path setup so we do NOT cd into operator.
set +e
uv run --quiet --with pyyaml python3 \
  "${REPO_ROOT}/operator/adapter/run_prod_smoke_test.py" \
  --customer "${SLUG}" \
  --app "${APP_NAME}" \
