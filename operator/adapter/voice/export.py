"""Voice library export pipeline (issue #862).
...
serializes the voice samples written by :mod:`adapter.voice.pipeline`
(PR #951) into the export archive.

The privacy floor from PR #951 is preserved end-to-end. The voice
ingestion pipeline never persists a raw email body; it stores a
structural-diff JSON per accepted message under
...
* **Read-only.** The export does not mutate ``voice_ingestion_items``
  ...
  ``voice_source_state`` row. Decommission (PR #956) does the deletion
  AFTER the export has been written to its archive destination.

* **Structural-diff only.** Each per-cohort voice sample is the
  privacy-bounded JSON produced by
  :func:`adapter.voice.diff.extract_structural_diff`. We copy bytes;
...
# Schema version for the export format itself. Sibling of
# memory/export.EXPORT_SCHEMA_VERSION; both move together by convention.
...
class VoiceR2ObjectReader(Protocol):
    """Per-customer R2 binding (read-only). See :class:`adapter.memory.export.R2ObjectReader`."""

    async def get(self, key: str) -> bytes: ...


class VoiceExportWriter(Protocol):
    """Same write contract as :class:`adapter.memory.export.MemoryExportWriter`.
...
class VoiceExportSigner(Protocol):
    """No-op signer seam. Mirrors :class:`adapter.memory.export.ExportSigner`."""
...
class NoOpVoiceExportSigner:
    """No-op signer; same shape as :class:`adapter.memory.export.NoOpExportSigner`."""
