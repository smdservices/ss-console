...
- `documents` - an array of `StoredDocument` shapes from `src/lib/operator/capabilities/document-storage.ts`. Each entry includes `id`, `path`, `filename`, `mime_type`, `size_bytes`, `created_at`, `modified_at`, `current_version`, `classification`.
