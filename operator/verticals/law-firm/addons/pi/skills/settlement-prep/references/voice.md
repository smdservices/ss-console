...
The voice gate harness (`operator/voice-gate/`, issue #855) scores the assembled prose along four axes:
...
A failing score causes the skill to emit a structured-table-only variant (matter-facts as a one-line table, chronology as a structured list, no narrative lead-ins). The partner authors the lead-ins. The fallback path is documented in `operator/voice-gate/voice-gate-fallback.md`.
