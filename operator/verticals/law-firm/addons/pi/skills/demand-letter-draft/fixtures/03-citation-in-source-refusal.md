...
The citation-refusal substrate (`operator/safety-substrate/citation_filter.py`) blocks any draft body containing a citation-shaped string. The skill's readiness rubric is the upstream check that prevents the skill from attempting a draft that the substrate would block. Refusing at the rubric layer is cheaper, surfaces a clearer error to the partner, and avoids a `block`-severity audit event when the same outcome can be a `SKILL_REFUSED` event.
