...
alongside `.yaml` fixtures under `operator/skills/<skill>/fixtures/`) match
...
Design constraints from issue #825:
  ...
  3. Captain bypass: `python -m operator.tests.skill_regression --regenerate <skill-slug>`
...
# the legacy operator/skills/ (shared skills) and the new vertical-pack
# location at operator/verticals/<vertical>/addons/<addon>/skills/. The
...
SKILL_ROOT_CANDIDATES: tuple[Path, ...] = (
    OPERATOR_ROOT / "verticals" / "law-firm" / "addons" / "pi" / "skills",
    OPERATOR_ROOT / "skills",
)
GOLDEN_ROOT_CANDIDATES: tuple[Path, ...] = (
    OPERATOR_ROOT / "verticals" / "law-firm" / "addons" / "pi" / "tests" / "golden",
...
def read_golden(golden_path: Path) -> dict:
    if not golden_path.exists():
        raise RegressionError(
            ...
            f"regenerate with `python -m operator.tests.skill_regression "
...
def write_markdown_report(results: list[FixtureResult], out_path: Path) -> None:
    ...
    if failed:
        lines.append(
            ...
            "`python -m operator.tests.skill_regression --regenerate <skill-slug>` "
            "and commit the updated `operator/tests/golden/<skill-slug>/*.json`."
        )
        lines.append("")
    lines.append(
        "<sub>Generated by `operator/tests/skill_regression.py` per "
