...
This file makes `pytest operator/tests/` (or `cd operator && pytest tests/`)
...
def test_fixture_matches_golden(skill_slug: str, fixture_name: str) -> None:
    ...
    assert actual == expected, (
        ...
        f"`python3 operator/tests/skill_regression.py --regenerate {skill_slug}` "
        f"followed by `npx prettier --write operator/tests/golden/`."
    )
