...
| Smokeball (`mcp:smokeball`)                      | `SMOKEBALL_REFRESH_TOKEN` in gateway env            | in-gateway            | **No** (client system of record)               | Must move behind the broker before a paying seat authors `code_execution`. Until then the guard refuses `code_execution` on any seat with Smokeball enabled. Tracked here; broker migration is the ADR 0045 step-7 build for this connector. The same subprocess also holds `SMOKEBALL_VISION_ANTHROPIC_KEY` (scanned-document transcription, ss#2464) - an SMD-side, spend-limited LLM key with no client-data authority, deliberately NOT the seat's `ANTHROPIC_API_KEY` because a connector's environment values are materialized to the per-seat volume. |
