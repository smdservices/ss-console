...
Source of truth for per-skill ship readiness across the 58-skill SMD AI Employee library. Updated continuously as new test runs land. Per the rubric at `rubric.md`, each skill's verdict resolves from the per-fixture audit trail in `runs/{skill}/`.
...
| AIEmployee adapter (trust_ceiling.py + aie_adapter.py) | ⏳ Logic complete + tested in fixtures; Hermes tool-dispatch wire deferred |
