...
Source of truth for how every skill in `ai-employee/skills/` is graded against its fixtures. The rubric must be operational (specific pass/fail conditions, not vibes) so the grading matrix at `ai-employee/grading/matrix.md` is defensible.
...
1. Run the skill against every fixture in `ai-employee/fixtures/<vertical>/<skill>/`.
...
Every test run logs the following to `ai-employee/grading/runs/<skill>/<fixture-id>-<timestamp>.json`:
...
```json
{
  ...
  "verdict_reasoning": "...",
  "safety_violations": [],
  "output_path": "ai-employee/grading/runs/inbox-triage/marketing-inbox-001-2026-05-20T16-34-11Z.md"
}
```
...
For each skill graded, the audit trail provides tokens + tool calls per fixture. The rollup script in `ai-employee/grading/rollup.py` (pending Phase E) sums these across a customer's enabled-skill set + estimated cadence (from the skill's SKILL.md frontmatter `cost_estimate`) to produce a monthly cost projection per customer.
