...
Custom MCP server that wraps the LawPay (now branded "8am") REST API and exposes it as MCP tools for Hermes. Required for the law-firm AI Employee vertical pack — LawPay is the de facto IOLTA-compliance payment standard for small/mid law firms (60,000+ firms, approved by all 50 state bars + ABA).

Per `docs/strategy/ai-employee-connector-coverage-2026-05-14.md`: Tier-1 BUILD, ~1-2 days, OAuth2 documented at developers.8am.com.

## What this wrapper exposes

The MCP tools provided correspond to the LawPay actions a law-firm AI Employee actually performs. Read-only by default; write actions are explicitly gated.
...
/opt/hermes/.venv/bin/python -m ai_employee_lawpay.server \
  --transport stdio \
  --customer-id smd
...
mcp_servers:
  lawpay:
    ...
    args: [-m, ai_employee_lawpay.server, --transport, stdio, --customer-id, smd]
    env:
...
1. Customer requests OAuth: `python -m ai_employee_lawpay.setup` (prints an authorization URL)
...
```bash
cd ai-employee/connectors/lawpay
