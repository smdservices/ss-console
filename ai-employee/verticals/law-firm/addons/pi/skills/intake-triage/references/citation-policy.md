...
3. **Defense in depth.** A separate code-level filter (`ai-employee/safety-substrate/citation_filter.py`) enforces the prohibition at the output layer. This document enforces it at the prompt layer. Two layers covering the same constraint means that if one fails the other catches it.
