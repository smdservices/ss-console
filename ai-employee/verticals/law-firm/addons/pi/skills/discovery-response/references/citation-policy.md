...
3. **The substrate's filter respects the carve-out.** The filter's `verbatim_quote_exempt` configuration flag (set at skill load time per `customer.yaml.skill_config.discovery-response.verbatim_quote_exempt: true`) instructs the filter to ignore citation-shaped strings inside the verbatim-quote markdown blocks. The carve-out is narrow: only the per-request table's request-text column, only the recital lead-in's verbatim-quote sub-clauses (e.g., a partner-supplied service-date quote). Every other section is fully subject to the prohibition.
