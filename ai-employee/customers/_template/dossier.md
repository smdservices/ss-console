...
> **Status:** template. Copy this file to `ai-employee/customers/{firm-slug}/dossier.md`, then fill in every bracketed field. Bracketed fields that remain after the runbook completes are pre-meeting blockers. Required by Platform PRD §16.2 + `docs/runbooks/pi-firm-demo-prep.md`.
...
> **Slug rules:** `^[a-z0-9][a-z0-9-]{0,31}$`, matches `ai-employee/customers/{firm-slug}/` and `customer.yaml` `customer_id`. The reserved `_template` directory is never a real customer slug.
...
> See `docs/specs/ai-employee/voice-ingestion.md` for what gets stored. The pipeline persists structural-diffs only, never raw text. Minimum bar before demo: **10 samples** sourced from public writing.
...
- [ ] `ai-employee/customers/{firm-slug}/customer.yaml` authored and validates against `docs/specs/ai-employee/customer-yaml-schema.md`
- [ ] `ai-employee/bin/provision-customer.sh {firm-slug}` completes successfully
- [ ] `ai-employee/bin/prepare-demo-firm.sh --firm-slug {firm-slug}` exits 0
