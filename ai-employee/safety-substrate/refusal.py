...
When `enforce()` in `ai-employee/adapter/trust_ceiling.py` returns
...
::
    ...
    from ai_employee_safety_substrate.refusal import (
        CustomerMessage,
        RefusalHandler,
        RefusalOutcome,
...
sys.path.insert(0, str(_HERE.parents[1]))  # ai-employee/
...
class CustomerMessage(str, enum.Enum):
    ...
    APPROVAL_REQUIRED_SEND = (
        "Your AI employee wanted to send a message but it needs your "
        "approval first. Open the draft to review and send."
    )
    APPROVAL_REQUIRED_COMMITMENT = (
        "Your AI employee paused before agreeing to something on your "
        "behalf. Open the request to review and approve."
    )
    APPROVAL_REQUIRED_DESTRUCTIVE = (
        "Your AI employee paused before doing something it cannot undo. "
        "Open the request to review and approve."
    )
    DESTRUCTIVE_BLOCKED_AT_DRAFT_CEILING = (
        "Your AI employee tried to delete or remove something, but its "
        "current ceiling does not allow that. Raise the ceiling or do "
        "it yourself if needed."
    )
    SKILL_DISABLED = (
        "Your AI employee tried to use a skill that is currently off. "
        "Re-enable the skill in Settings if you want it to run."
    )
    UNKNOWN_ACTION = (
        "Your AI employee tried something the substrate did not "
        "recognize. We have logged it for review."
    )
    GENERIC_REFUSED = (
        "Your AI employee tried an action that was not allowed under "
...
def _message_for(reason: DecisionReason) -> CustomerMessage:
    ...
    log.warning(
        ...
        "ai-employee/safety-substrate/refusal.py.",
        reason.value,
    )
