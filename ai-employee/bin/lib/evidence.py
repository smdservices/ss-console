"""CLI entrypoint for ai-employee/bin/generate-evidence-packet.sh (#894).
...
# ai-employee/ on sys.path so `from adapter.evidence import ...` resolves.
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "custody). Defaults to "
            "ai-employee/customers/<slug>/.evidence-audit.sqlite"
        ),
    )
    p.add_argument(
        "--read-db",
...
def _resolve_paths(args: argparse.Namespace) -> tuple[Path, Path, Path]:
    ...
    customer_dir = repo_root / "ai-employee" / "customers" / args.customer
