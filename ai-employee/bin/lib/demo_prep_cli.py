...
Invoked from ``ai-employee/bin/prepare-demo-firm.sh``. The shell script
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Pre-meeting readiness checks for a PI firm AI Employee demo. "
    ...
    parser.add_argument(
        "--firm-slug",
        required=True,
        help="Customer slug to validate. Matches the directory name under ai-employee/customers/.",
    )
    parser.add_argument(
        "--customers-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "against. Repeatable. Defaults to ai-employee/skills/ and "
            "ai-employee/fixtures/ when omitted."
        ),
    )
    parser.add_argument(
        "--min-voice-samples",
...
def _resolve_aie_root(explicit_root: Path | None) -> Path:
    """Locate ``ai-employee/`` relative to this script.

    The CLI is invoked via ``uv run --quiet --with pyyaml python3 -m bin.lib.demo_prep_cli``
    from inside ``ai-employee/``, matching the decommission CLI's pattern.
    The explicit override is for tests.
    """
    if explicit_root is not None:
        return explicit_root.resolve()
    here = Path(__file__).resolve()
    # ai-employee/bin/lib/demo_prep_cli.py -> ai-employee/
