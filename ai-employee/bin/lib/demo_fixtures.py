...
* **No real-customer pollution at the file-system level either.** The
  ...
  ``ai-employee/customers/{slug}/.demo-fixtures-state.json`` and (when
...
# ``ai-employee/customers/{slug}/.demo-fixtures-state.json``, mirroring
...
def parse_pi_matter(path: Path) -> PIMatterEnvelope:
    ...
    The matter format is established by ``ai-employee/fixtures/law-
...
def load_pi_inputs(fixtures_root: Path) -> PILoaderInputs:
    ...
    ``fixtures_root`` is ``ai-employee/fixtures/`` resolved by the CLI.
    ...
    def _preflight_customer_dir(self) -> Path:
        ...
        if not customer_dir.is_dir():
            raise DemoFixturePreflightError(
                ...
                "copy ai-employee/customers/_template/ to populate it first"
            )
