 ...
 * (entity, 'operator') if provided — verified by set_pto() before
...
async function validateBackupUser(
  ...
  const roles = await listProductRoles(db, args.backupUserId, args.entityId, 'operator')
