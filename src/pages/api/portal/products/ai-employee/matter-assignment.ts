...
/**
 * POST /api/portal/products/ai-employee/matter-assignment
 *
 ...
 *     (entity, 'ai-employee') — operators can re-assign too because
...
function matterUrl(matterId: string, status: string): string {
  const path = `/portal/products/ai-employee/matters/${encodeURIComponent(matterId)}`
...
function redirectListWithStatus(status: string): Response {
  return new Response(null, {
    status: 303,
    headers: {
      Location: `/portal/products/ai-employee/matters?status=${encodeURIComponent(status)}`,
    },
  })
}
 ...
 * customer's AI Employee subscription.  A user without any role on
 * (entity, 'ai-employee') cannot be assigned matters — the routing
 * layer would have no permission to surface the matter to them.
 */
async function targetHasAiEmployeeAccess(userId: string, entityId: string): Promise<boolean> {
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (parsed.action === 'assign') {
    const hasAccess = await targetHasAiEmployeeAccess(parsed.userId, ctx.client.id)
