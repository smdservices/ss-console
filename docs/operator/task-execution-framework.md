...
- **Primitive.** `operator_handoff_task` (console `tools.ts:168` → `/webhooks/handoff`, `webhook_gate.py:780`). **Status: PARTIAL / fail-closed.** Verified: the gate authenticates and would 202, but with no `handoff` `webhook_trigger` in any `customer.yaml`, `translate.py` materializes no route and Hermes 404s the delivery; the gate 503s "handoff route not configured" when the secret is set (`webhook_gate.py:795`). Hermes `/background` (in-memory asyncio) and the kanban queue (Fly-volume SQLite) exist but **neither survives Machine replacement.**
