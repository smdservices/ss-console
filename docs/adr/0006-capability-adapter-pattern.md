...
- The prior ADR placed capability interfaces at `operator/capabilities/{name}.ts` and described adapters that implement them; it implied the runtime would enforce the interface boundary via the SMD overlay's `aie_adapter.register()` against a Hermes overlay surface (`smd.hooks.capability_adapter`).
...
### Capability interfaces — TypeScript types at `src/lib/operator/capabilities/`
