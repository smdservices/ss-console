...
| Email                            | Google Workspace (Gmail)                 | `build:google-gmail` (`operator/connectors/google/crane_gmail.py`)                            | BUILD, not the Preview Google MCP. Scope-limited user-OAuth token (`gmail.modify` — read/label/archive/draft, **no send**) on the Fly volume (ADR 0010). The token scope is the hard wall (ADR 0035), not verb minimalism. Google's Gmail MCP is Developer-Preview/not-GA and the overlay has no OAuth-MCP wiring path. Live since customer-zero v1. See §"Google Workspace: BUILD, not MCP". |
| Calendar                         | Google Workspace                         | `build:google-calendar` (`operator/connectors/google/crane_calendar.py`)                      | BUILD. Scope `calendar.events`; read + draft-event (`sendUpdates="none"`, no external attendees — reviewer-as-sender). Same Preview / no-OAuth-MCP reasons; one shared Google token.                                                                                                                                                                                                          |
| DocumentStorage                  | Google Drive + Docs                      | `build:google-drive` (`operator/connectors/google/crane_drive.py`)                            | BUILD. Scopes `drive.readonly` + `drive.file`; read/export (incl. Docs) + app-owned draft Docs, **no external share**. `drive.readonly` is whole-Drive read — the highest-blast-radius scope in the v1 set, authored deliberately.                                                                                                                                                            |
...
The three Google rows above were `mcp:google-*` in the original table. They are now `build:` adapters (`operator/connectors/google/crane_{gmail,calendar,drive}.py`), decided during the customer-zero connector session. Three reasons, in order of weight:

1. **Governability via scope-limited credentials.** The hard security boundary is the OAuth token scope, not the verbs an adapter exposes — `execute_code` can read the token on the Fly volume and call Google at its full granted scope, so verb minimalism is ergonomics, scope is the wall (this updates any earlier "structurally cannot send" framing per [ADR 0035](./0035-no-imposed-entitlement-defaults.md): the constraint is the _authored_ scope, not a harness default). The native Hermes `google-workspace` skill mints one token with `gmail.send` + full `calendar` + full `drive` write and exposes send/share as free-form bash; we instead mint exactly `gmail.modify` + `calendar.events` + `drive.readonly` + `drive.file`. The native `google-workspace` and `himalaya` skills are disabled per-customer (`skills_disabled`) for the same reason.
...
3. **One owned credential path, already proven.** `crane_gmail.py` has run live since customer-zero v1 on a Google authorized-user token at `/opt/data/oauth/google.json` (ADR 0010). Calendar and Drive/Docs ride the **same** token (shared `_google_auth.py`), minted once via the extended `operator/bin/gmail-oauth-consent.py`. The capability contracts (`src/lib/operator/capabilities/{calendar,document-storage}.ts`) + the connector conformance suite (`operator/connectors/google/tests/`) hold the BUILD adapters to the same no-autonomous-send / no-fabrication invariants as the canonical filevine adapter.
