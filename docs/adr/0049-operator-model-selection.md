...
- **Live today, no build:** main model per customer (`customer.yaml` `model:` → `translate.py`). This is the primary lever and covers most seats.
- **Small, optional, build-when-needed:** for a seat with occasional genuinely-heavy reasoning — (a) express the delegation model in `customer.yaml`; (b) teach `translate.py` to emit the native `delegation` block; (c) author the two or three heavy-reasoning skills to `delegate_task` up. None of this is required for a seat whose main model is already capable enough.
