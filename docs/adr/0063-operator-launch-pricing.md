---
title: Operator Launch Pricing — Retainer + Stand-Up (figures private)
date: 2026-07-04
status: accepted
...
**Source:** ADR 0004 locked the pricing _shape_ (flat-rate monthly retainer, not metered, not credit-based) and deferred the number to a stack cost analysis. That analysis never landed a committed artifact; a working baseline (2026-06-10) lived only in session memory. The Review 5 unit-economics stress test (2026-07-03) produced the committed cost model this decision prices against, and the ADR 0062 cost plane made per-seat COGS observable — which also armed the need for a real price: the locked COGS/MRR kill criterion (spend > 40% of MRR for two consecutive months) is structurally inert while `services.recurring_price` is NULL.

## Decision

**The Operator launch price is a flat-rate monthly retainer plus a one-time stand-up fee.** The two figures are internal and live in `venturecrane/engagements:pricing/operator-launch-pricing.md`, which carries this ADR verbatim with the numbers intact. Internal, not published (the no-published-dollar-amounts rule stands; the client sees the price in their proposal).

## Derivation

- **Cost model (Review 5, committed 2026-07-03):** per-seat hard costs are a small fixed infrastructure line plus modeled token spend (now observable per seat via ADR 0062 workspace attribution). The dominant cost is Captain labor at the loaded rate: launch-level supervision (~16 h/mo) exceeds every hard cost combined. The prior baseline was therefore a **cost floor**, not a price — pricing at it is break-even under honest labor accounting. Figures: `venturecrane/engagements:pricing/operator-launch-pricing.md`.
- **Salary anchor (ADR 0037 tenet 1 / ADR 0040):** the Operator competes with a hire, never a software seat. The displaced coordinator role runs ~$60k/yr fully loaded before turnover cost, and the retainer prices _at_ that anchor: the buyer trades a salary for a seat that does not quit, with the managed service included.
