...
- [0015-hermes-fork-vs-upstream.md](./0015-hermes-fork-vs-upstream.md) - Hermes fork vs upstream-PR: thin vendored fork at `venturecrane/hermes-agent` with SMD overlay layer for safety-substrate hooks, plus active upstream contribution; rules out upstream-only and hard-fork
- [0016-honcho-disposition.md](./0016-honcho-disposition.md) - Honcho disposition: proposer-only mode in the overlay; observations land in `persona_observations`, never reach runtime persona state without an explicit calibration-session promotion that opens a `customer.yaml` PR
- [0017-skill-curator-disposition.md](./0017-skill-curator-disposition.md) - Autonomous Skill Curator disposition: observer-only mode in the overlay; drafts land in `skill_drafts`, never mutate runtime skill set; promotion is a `crane-console/.agents/skills/` PR; no self-promotion ever, not even in Phase 2
- [0018-gepa-disposition.md](./0018-gepa-disposition.md) - GEPA self-evolution disposition: disabled in customer Machines (boot-time disable verification); no per-customer observer surface; platform-level trace-analysis tooling deferred to a future ADR when need is concrete
