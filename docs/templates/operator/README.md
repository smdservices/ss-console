...
- [ADR 0035 No imposed entitlement defaults](../../adr/0035-no-imposed-entitlement-defaults.md) and [ADR 0025 Autonomy ceilings (configurable exposure vs. initiation)](../../adr/0025-autonomy-ceilings-configurable-exposure-vs-initiation.md): the harness imposes no default posture; entitlement is authored per action class and fail-closed when unauthored. Draft-for-review send ([ADR 0035](../../adr/0035-no-imposed-entitlement-defaults.md)) is one authored option, pinned as a non-raisable floor for regulated verticals (the law pack), not an architectural absolute. Any contract clause describing the send posture must reflect this; do not reintroduce "the Operator architecturally cannot send" language.
