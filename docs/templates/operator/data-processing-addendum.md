...
4.4 **Authored send posture.** The Operator's external-send and identity posture is governed by the Entitlement Configuration Customer authors under the Service Agreement (Section 2; [ADR 0025](../../adr/0025-autonomy-ceilings-configurable-exposure-vs-initiation.md), [ADR 0035](../../adr/0035-no-imposed-entitlement-defaults.md)). The enforcement mechanism is architectural: the configuration is enforced in code by the per-Customer trust-ceiling gate, audited, and not raisable by the Operator. The posture itself is what Customer authors. Where Customer authors `draft_for_review` for an external-send class (and where the applicable regulated-vertical pack pins draft_for_review as a non-raisable floor, as the law pack does), no outbound customer-bound message in that class is sent under the Operator's identity: every such message is drafted into a Reviewer's drafts folder for the Reviewer to send under the Reviewer's own identity, as documented in [ADR 0005](../../adr/0005-external-send-identity.md).
