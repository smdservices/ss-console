...
| `[EMAIL IDENTITY PROVIDER]`         | The agent email identity infrastructure provider for the persona's platform-provided mailbox(es).      |
...
4.4 **Authored send posture; reviewer-as-sender.** The Operator's external-send and identity posture is governed by the Entitlement Configuration Customer authors under the Service Agreement (Section 2; [ADR 0025](../../adr/0025-autonomy-ceilings-configurable-exposure-vs-initiation.md), [ADR 0035](../../adr/0035-no-imposed-entitlement-defaults.md)). The enforcement mechanism is architectural: the configuration is enforced in code by the per-Customer trust-ceiling gate, audited, and not raisable by the Operator. The posture itself is what Customer authors. Where Customer authors `draft_for_review` for an external-send class (and where the applicable regulated-vertical pack pins reviewer-as-sender as a non-raisable floor, as the law pack does), no outbound customer-bound message in that class is sent under the Operator's identity: every such message is drafted into a Reviewer's drafts folder for the Reviewer to send under the Reviewer's own identity, as documented in [ADR 0005](../../adr/0005-reviewer-as-sender.md).
...
5.1 **Authorized Sub-processors at the Effective Date.** Customer authorizes the following platform Sub-processors:
...
| **[EMAIL IDENTITY PROVIDER]** | Agent email identity infrastructure for the persona's platform-provided mailbox(es).                                          | Inbound and internal messages addressed to or from the persona's mailbox(es); message metadata. |

**Connector Sub-processors.** The Operator connects to Customer-authorized third-party systems through per-customer MCP servers (vendor-direct or vetted-community) and SMD-maintained connector adapters, using Customer's own OAuth tokens or API keys as the isolation primitive per [ADR 0020](../../adr/0020-connector-strategy.md) and [ADR 0010](../../adr/0010-per-customer-oauth-token-storage.md). SMD does not broker connector access through a shared-tenancy connector platform. The specific connector Sub-processors for this engagement are those corresponding to the connectors listed in Exhibit A to the Service Agreement; each is engaged on the terms of Section 5.2 and may be updated as Customer authorizes or removes connectors.
