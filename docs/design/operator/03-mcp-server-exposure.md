...
**Status:** Draft for Captain review (rev 4, 2026-06-14). Phase-1 hosting is console-mediated: the MCP server runs in the ss-console Worker, and the on-Machine sidecar becomes the documented durable alternative rather than the Phase-1 default. Auth Server is Clerk. rev 4 folds in a design-level security review of the console-hosted surface: customer routing must derive from the verified token (never a caller-supplied slug), the per-customer webhook secret, console-egress consent enforcement, and the new shared-host risk rows.
...
**The load-bearing isolation invariant (console hosting).** Because one Worker serves every customer, "which customer's data" is no longer answered by physical single-tenancy (a sidecar answers it by _which Machine you reached_); it is an application-code authz decision. The invariant: **the customer is derived from the verified Clerk token `aud`, and the per-customer HMAC read key and webhook secret are derived from that token-resolved slug.** Any customer slug appearing in the request path or body is treated as untrusted input and must be _checked to equal_ the token-resolved slug, never trusted to select the customer. A request whose path/body slug disagrees with its token is refused (audited `MCP_AUTH`), and a token whose `aud` does not match the resolved customer is refused before any data access. This is the confused-deputy weakness (the broker's known soft spot) one layer up; the console must close it the same way the broker does — authorize against the verified identity, not a caller-supplied parameter.
...
| **Customer-routing confused deputy** (caller-supplied slug selects the wrong customer in the shared Worker) | Customer is derived from the verified token `aud`; the per-customer HMAC read key and webhook secret derive from the token-resolved slug; any path/body slug is checked-to-match, never trusted to select (§2.1). Mismatch → `MCP_AUTH` refusal.                                                                                                                                                                                                                                                                                                                         |
