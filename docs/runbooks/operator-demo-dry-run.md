...
**Companion runbooks:** [`pi-firm-demo-prep.md`](./pi-firm-demo-prep.md) (#819) for the 24-48 hour pre-provisioning. [`ai-employee-customer-onboarding.md`](./ai-employee-customer-onboarding.md) for what happens after the meeting closes signed.
...
- [ ] Filevine OAuth: open the matter list in the dashboard. The fixture matters render. If Filevine OAuth has expired (token rotated, refresh failed per [`docs/specs/ai-employee/oauth-lifecycle.md`](../specs/ai-employee/oauth-lifecycle.md)), the matter list shows the empty-state stub per [`docs/style/empty-state-pattern.md`](../style/empty-state-pattern.md) and Captain has 60 minutes to re-auth or pivot to the synthetic-adapter no-PM angle (per [pi-firm-demo-prep.md recovery paths](./pi-firm-demo-prep.md#recovery-paths)).
...
- [ ] Anthropic API budget under the per-customer-per-day cap. (Hard caps enforced in code per [ai-employee-pricing-2026-05-13.md](../strategy/ai-employee-pricing-2026-05-13.md) risk: "Token spend escalation under bad agent loops.")
...
- [ ] Voice gate state is Pass or Near-pass (per [`docs/specs/ai-employee/voice-gate-fallback.md`](../specs/ai-employee/voice-gate-fallback.md)). If Fail, the Approve & Send affordance is disabled for external-send skills; the demo loses Scene 4 and Captain must pivot to a structural narration (per the Scene 4 failure recovery below).
...
### Scene 1: AI Employee landing (≈3 minutes)

**Target duration:** 2-3 minutes.

**On screen:** The portal landing surface for AI Employee. Persona card (Marcus, AI Associate, signature preview, avatar). Configured-for-{firm-name} confirmation. Connector status row (Outlook green, Filevine green or synthetic, DocuSign green). Skill set (4 enabled). Voice gate state badge (Pass or Near-pass).

**Captain narrates:** "This is what your AI Employee landing looks like. Marcus is configured for {firm-name}, the connectors are green, four skills are running. Today I'll show you what he's doing, what he's drafted, what he used to write it, how you approve and send, where the audit trail lives, and how you control any of it from settings. Then we'll talk about what would change for your firm specifically."
...
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "Who decided on the name Marcus?" | "That's a placeholder. You name your AI Employee anything you want; it'd live on every draft and the internal inbox display."                                                                                                                                                                                        |
...
- **Persona card renders empty-state stub.** Means customer.yaml `personas[]` was unpopulated at provision time (per [day-1-onboarding.md](../specs/ai-employee/day-1-onboarding.md) Screen 2 prerequisite). This should have been caught in the pre-meeting checklist; if it surfaces in front of the partner, acknowledge openly: "The persona card isn't rendering; let me show you the drafts list and we'll come back to settings later." Move to Scene 2.
...
**On screen:** The settings surface for AI Employee. Sub-tabs: Persona, Voice (samples + gate state + cohort histogram), Skills (4 enabled rows with trust-ceiling controls), Connectors, Reviewers (identity binding), Memory (rules + seed-time provenance), Digest (cadence). For the rehearsal, open Skills first.
...
| "Who can change settings?"                                        | "Per the dashboard-roles spec: principal users (partners) can change anything. Operator users (paralegals) can adjust queue and memory, view settings but not promote trust ceilings. Compliance users have read-only audit access." (Per [dashboard-roles.md](../specs/ai-employee/dashboard-roles.md).) |
...
- **Settings page renders empty for a sub-tab.** Likely the customer.yaml field wasn't populated at provision time. Acknowledge per [day-1-onboarding.md](../specs/ai-employee/day-1-onboarding.md) per-screen gating: "The {sub-tab} is empty on this fixture; in production it'd be populated from your customer.yaml configuration. Let me show you the next sub-tab." Skip to the next working sub-tab.
...
| "What if the AI Employee isn't producing value at day ninety?"                | "You exit clean. Thirty-day notice, no penalty, no balance-of-contract payment. The evaluation window is the explicit place that decision happens; we want it to be a real choice, not a stuck-in-a-bad-fit problem."                                                                                                                              |
| "What if we want to bring an AI Employee for a different vertical (not law)?" | "Each vertical pack is a separate engagement at the same flat tier; you'd have two AI Employees, two retainers, two customer.yaml configurations. We'd discuss whether vertical-pack overlap (e.g., compliance, audit) lets one persona serve multiple business areas, but the default is one persona per vertical."                               |

**Failure recovery:**

- **Pricing doc is mid-revision.** If the launch pricing has been revised since this runbook was last touched, fall back to the most recent [ai-employee-pricing-2026-05-13.md](../strategy/ai-employee-pricing-2026-05-13.md) state on main; do not quote a number that isn't in the source document. Pricing is the one place fabrication is unforgivable.
...
- **Partner hard-asks for month-to-month or a discounted first month.** Hold the line per [pricing-doc revision triggers](../strategy/ai-employee-pricing-2026-05-13.md#revision-triggers); document the request, do not concede live. The price-value equation is what we revise across customers, not what we negotiate in a single meeting.
...
- If the partner signaled interested: send the engagement letter draft within 24 hours per [`docs/runbooks/ai-employee-customer-onboarding.md`](./ai-employee-customer-onboarding.md) §0.
- If the partner signaled lukewarm: send a brief follow-up within 48 hours with a 12-minute video walkthrough link (per [day-1-onboarding.md](../specs/ai-employee/day-1-onboarding.md) Captain-led path); leave the door open without re-pitching.
...
- [`ai-employee-customer-onboarding.md`](./ai-employee-customer-onboarding.md): what happens after sign.
- [`docs/specs/ai-employee/day-1-onboarding.md`](../specs/ai-employee/day-1-onboarding.md) (#803): Day-1 screen sequence; Captain-led walk-through cadence section.
- [`docs/specs/ai-employee/voice-gate-fallback.md`](../specs/ai-employee/voice-gate-fallback.md): voice gate Pass / Near-pass / Fail behavior referenced in Scenes 3, 4, 7.
- [`docs/specs/ai-employee/capability-contracts.md`](../specs/ai-employee/capability-contracts.md): Pattern A reviewer-as-sender architecture referenced in Scene 4.
- [`docs/specs/ai-employee/audit-log-immutability.md`](../specs/ai-employee/audit-log-immutability.md): hash-chained append-only log referenced in Scene 6.
- [`docs/specs/ai-employee/compliance-evidence-packet.md`](../specs/ai-employee/compliance-evidence-packet.md): packet structure referenced in Scene 6.
- [`docs/specs/ai-employee/dashboard-roles.md`](../specs/ai-employee/dashboard-roles.md): principal / operator / compliance role visibility referenced in Scene 7.
- [`docs/specs/ai-employee/fabrication-filter.md`](../specs/ai-employee/fabrication-filter.md): pre-queue fabrication rejection referenced in Scene 3.
- [`docs/strategy/ai-employee-pricing-2026-05-13.md`](../strategy/ai-employee-pricing-2026-05-13.md) (#794): pricing structure for Scene 8.
- [`docs/pm/ai-employee/law-firm-prd.md`](../pm/ai-employee/law-firm-prd.md) §11: walk-in-cold demo strategy; structure, set-pieces, compliance moment, order-taking moment.
