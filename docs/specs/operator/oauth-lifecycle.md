...
6. Customer clicks the link, completes OAuth consent in their browser. Callback hits `https://portal.smd.services/operator/oauth/{connector}/callback` (portal subdomain; customer-facing). The portal callback handler exchanges the code and relays the resulting token by setting the customer app's Fly secret and restarting the Machine (the boot-decode writes `/opt/data/oauth/{connector}.json`); see [ADR 0036](../../adr/0036-oauth-token-relay-fly-secret-restart.md). The admin subdomain stays role-gated for SMD operations only.
...
| ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | --------------------------------- | ---------------- |
| microsoft-graph (Email)                                                         | `Mail.Read`, `Mail.ReadWrite`, `MailboxSettings.Read`, `Mail.Send` (requested when send is authored; ADR 0035) | 1 hr access / 90 day idle refresh | At 75 days idle  |
...
- Astro route at `src/pages/portal/products/operator/oauth/[connector]/callback.ts` handles OAuth callbacks on the portal subdomain (customer-facing); relays the exchanged token via Fly app secret + Machine restart ([ADR 0036](../../adr/0036-oauth-token-relay-fly-secret-restart.md)). The sibling `[connector]/index.ts` is the connect-initiate route. Admin subdomain stays role-gated.
