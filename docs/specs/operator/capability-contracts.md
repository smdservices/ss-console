...
**Send is configured per engagement, never assumed.** Earlier drafts of this spec locked "Pattern A only" — agent drafts, reviewer always sends — as an architectural invariant. That is the dead default struck by [ADR 0035](../../adr/0035-no-imposed-entitlement-defaults.md) (amending [ADR 0005](../../adr/0005-external-send-identity.md) / [ADR 0025](../../adr/0025-autonomy-ceilings-configurable-exposure-vs-initiation.md)). The current holding:

- An adapter **may** expose a send method. Whether a send executes autonomously, routes to a reviewer draft, or is refused is decided **at runtime** by `trust_ceiling.enforce()` per the authored `EXTERNAL_SEND` ceiling, **fail-closed when unauthored** (no send, no draft).
- The draft-for-review external send posture is **one authored option**, not the default — and a regulated-vertical pack may pin it as a non-raisable floor. The capability layer does not enforce it by omitting `send`; the trust ceiling enforces the authored posture.
