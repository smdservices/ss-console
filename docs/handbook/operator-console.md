...
- **Settings** (`settings/`) - the ACT surface (console blueprint §5, amended 2026-07-15): Plan & billing (the subscription status under the SMD-only `provisioning` domain, plus the Stripe Manage Billing entry - the commercial plane lives here, never on the read page) and the entries to Connections, Team access, and Advanced. Also the redirect target for the customer-facing OAuth connect flow (`?status=connected` on success, `?status=failed&reason=<short>` on failure, rendered as a banner with actionable copy per failure reason). Future config domains grow sections here as each becomes self-serve.
