...
- 1-2 PI-specialized skills from the law-firm vertical pack: minimum is `pi-intake-triage`. Demand-letter category v1 uses `pi-demand-letter-evidence-packet` (assembles the inputs a partner needs to draft, no legal-judgment authoring); `pi-demand-letter-text-only` deferred to Phase 3+ per law-firm-prd.md §6.2
