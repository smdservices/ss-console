# Marketing — UX Brief (Swiss Functional)

_First-pass brief, authored 2026-04-19 alongside the identity sweep. Scope: the `smd.services` apex (public) surfaces — homepage, booking flow, get-started, scorecard, contact, 404, and the sign-in pages. Every surface inherits the Swiss Functional identity committed in `.design/DESIGN.md`._
...
- **Hero.** Oversized Switzer headline, one-line tagline in Switzer, single CTA. No rotator, no gradient text, no animated keyword reveal. The headline does the work.
